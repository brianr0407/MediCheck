#pragma once
#include "frmAgregarMedicamento.h"
#include "frmEliminarMedicamento.h"
#include "frmModificarMedicamento.h"
...
namespace VIEW {
	...
	public ref class frmMantMedicamentos : public System::Windows::Forms::Form
	{
	public:
		frmMantMedicamentos(void)
		{
			InitializeComponent();
			this->medicamentoController = gcnew MedicamentoController();
			this->ControlBox = false;
		}
		frmMantMedicamentos(MedicamentoController^ medicamentoController, Medicamento^ medicamento)
		{
			InitializeComponent();
	...
	private: System::Windows::Forms::DataGridViewTextBoxColumn^ Column5;
		   ...
		   // ── Paleta de colores ─────────────────────────────────────────────────
		   // Azul marino oscuro  : Color::FromArgb(15, 41, 66)   → header / btn buscar
		   ...
		   // Selección bg        : Color::FromArgb(210, 228, 248)
		   // Selección fg        : Color::FromArgb(12, 52, 100)
		   // ─────────────────────────────────────────────────────────────────────
...
#pragma region Windows Form Designer generated code
		   void InitializeComponent(void)
		   {
			   ...
			   // Ancho = 380px sobre 963px total ≈ 40%
			   // Los controles internos se estiran a 340px (380 - 2*20 de margen)
			   ...
			   this->panelBusqueda->Controls->Add(this->comboBox2);
			   ...
			   this->lineaBusqueda->Size = System::Drawing::Size(340, 1);  // estira con el panel
			   ...
			   this->comboBox1->Size = System::Drawing::Size(340, 25);  // ← estirado
			   ...
			   this->textBox2->Size = System::Drawing::Size(340, 24);  // ← estirado
			   this->textBox2->TabIndex = 5;
			   this->textBox2->Text = L"Ej. amoxisilina";
			   this->textBox2->ForeColor = Color::FromArgb(180, 190, 200);
			   this->textBox2->Enter += gcnew System::EventHandler(this, &frmMantMedicamentos::textBox2_Enter);
			   this->textBox2->Leave += gcnew System::EventHandler(this, &frmMantMedicamentos::textBox2_Leave);
			   ...
			   this->comboBox2->ForeColor = Color::FromArgb(30, 50, 70);
			   ...
			   this->comboBox2->Location = Point(20, 192);
			   this->comboBox2->Name = L"comboBox2";
			   this->comboBox2->Size = System::Drawing::Size(340, 25);  // ← estirado
			   ...
			   this->button4->Size = System::Drawing::Size(340, 36);  // ← estirado
			   this->button4->TabIndex = 8;
			   this->button4->Text = L"Buscar";
			   this->button4->UseVisualStyleBackColor = false;
			   this->button4->Click += gcnew System::EventHandler(this, &frmMantMedicamentos::button4_Click);

			   // ── panelResultados ──────────────────────────────────────────────
			   // Padding(16,8,16,0) → 16px margen izq/der, 8px arriba, 0 abajo
			   ...
			   this->dataGridView1->CellContentClick += gcnew DataGridViewCellEventHandler(this, &frmMantMedicamentos::dataGridView1_CellContentClick);
			   ...
			   this->Column5->HeaderText = L"Especificaciones";
			   ...
			   this->button2->Click += gcnew System::EventHandler(this, &frmMantMedicamentos::button2_Click);
			   ...
			   this->button3->Click += gcnew System::EventHandler(this, &frmMantMedicamentos::button3_Click);
			   ...
			   this->button1->Click += gcnew System::EventHandler(this, &frmMantMedicamentos::button1_Click);
			   ...
			   this->button5->Click += gcnew System::EventHandler(this, &frmMantMedicamentos::button5_Click);
			   ...
			   this->Name = L"frmMantMedicamentos";
			   this->Text = L"";
			   this->Load += gcnew System::EventHandler(this, &frmMantMedicamentos::frmMantMedicamentos_Load);
...
#pragma endregion
		   ...
		   // ── LÓGICA (sin cambios) ──────────────────────────────────────────────
	private: void mostrarGrilla(List<Medicamento^>^ listaMedicamentos) {
		this->dataGridView1->Rows->Clear();
		for (int i = 0; i < listaMedicamentos->Count; i++) {
			Medicamento^ med = listaMedicamentos[i];
			Antibiotico^ ab = dynamic_cast<Antibiotico^>(med);
			SuplementoVitaminico^ sv = dynamic_cast<SuplementoVitaminico^>(med);

			array<String^>^ fila = gcnew array<String^>(5);
			fila[0] = med->getId().ToString();
			fila[1] = med->getNombre();
			fila[2] = med->getTamano();
			fila[3] = (ab != nullptr) ? "Antibiotico" : "Suplemento";

			String^ text;
			if (ab != nullptr)
				text = ab->getEsPrioridadCritica() ? "Es Prioridad" : "No es Prioridad";
			fila[4] = (ab != nullptr)
				? text + "(" + ab->getEsPrioridadCritica().ToString() + ")"
				: "Vitamina " + sv->getTipo();

			this->dataGridView1->Rows->Add(fila);

			// Alternar color de filas (par = azul muy claro, impar = blanco)
			if (i % 2 == 0)
	...
	private: System::Void frmMantMedicamentos_Load(System::Object^ sender, System::EventArgs^ e) {
		mostrarGrilla(this->medicamentoController->ObtenerTodosMedicamentosTXT());
	}
	private: System::Void button3_Click(System::Object^ sender, System::EventArgs^ e) {
		mostrarGrilla(this->medicamentoController->ObtenerTodosMedicamentosTXT());
	}
	private: System::Void button4_Click(System::Object^ sender, System::EventArgs^ e) {
		List<Medicamento^>^ todos = this->medicamentoController->ObtenerTodosMedicamentosTXT();
		...
		String^ filtroTamano = this->comboBox2->Text->Trim()->ToUpper();

		for each (Medicamento ^ med in todos) {
			bool coincide = true;
			if (filtroId != "" && med->getId().ToString() != filtroId)                  coincide = false;
			if (filtroNombre != "" && !med->getNombre()->ToUpper()->Contains(filtroNombre)) coincide = false;
			if (filtroTamano != "" && !med->getTamano()->ToUpper()->Contains(filtroTamano)) coincide = false;
			if (coincide) resultados->Add(med);
		}
		mostrarGrilla(resultados);
	}
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		if (this->dataGridView1->SelectedRows->Count == 0) {
			MessageBox::Show("Seleccione un medicamento primero", "Aviso",
				MessageBoxButtons::OK, MessageBoxIcon::Warning);
			return;
		}
		int filaSeleccionada = this->dataGridView1->SelectedRows[0]->Index;
		int id = Convert::ToInt32(this->dataGridView1->Rows[filaSeleccionada]->Cells[0]->Value->ToString());
		Medicamento^ medicamento = this->medicamentoController->BuscarMedicamentoPorIdTXT(id);

		frmEliminarMedicamento^ ventana = gcnew frmEliminarMedicamento(this->medicamentoController, medicamento);
		ventana->ShowDialog();
		mostrarGrilla(this->medicamentoController->ObtenerTodosMedicamentosTXT());
	}
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		frmAgregarMedicamento^ ventana = gcnew frmAgregarMedicamento(this->medicamentoController);
		ventana->ShowDialog();
		mostrarGrilla(this->medicamentoController->ObtenerTodosMedicamentosTXT());
	}
	private: System::Void button5_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		frmModificarMedicamento^ ventana = gcnew frmModificarMedicamento(this->medicamentoController, medicamento);
		ventana->ShowDialog();
		mostrarGrilla(this->medicamentoController->ObtenerTodosMedicamentosTXT());
	}
	...
	private: System::Void textBox2_Enter(System::Object^ sender, System::EventArgs^ e) {
		if (this->textBox2->Text == L"Ej. amoxisilina") {
			this->textBox2->Text = L"";
			this->textBox2->ForeColor = Color::FromArgb(30, 50, 70);  // color normal
		}
	}
	...
	private: System::Void textBox2_Leave(System::Object^ sender, System::EventArgs^ e) {
		if (this->textBox2->Text->Trim() == L"") {
			this->textBox2->Text = L"Ej. amoxisilina";
			this->textBox2->ForeColor = Color::FromArgb(180, 190, 200);  // vuelve al gris
		}
	}
	};
}