#pragma once
#include "frmAgregarAlarma.h"
#include "frmEliminarAlarma.h"
#include "frmModificarAlarma.h"
...
namespace VIEW {
	...
	public ref class frmMantAlarma : public System::Windows::Forms::Form
	{
	public:
		frmMantAlarma(void)
		{
			InitializeComponent();
			this->alarmaController = gcnew AlarmaController();
			this->ControlBox = false;
		}
		frmMantAlarma(AlarmaController^ alarmaController)
		{
			InitializeComponent();
...
#pragma region Windows Form Designer generated code
		   void InitializeComponent(void)
		   {
			   ...
			   this->textBox2->Text = L"Ej. Activa";
			   this->textBox2->ForeColor = Color::FromArgb(180, 190, 200);
			   this->textBox2->Enter += gcnew System::EventHandler(this, &frmMantAlarma::textBox2_Enter);
			   this->textBox2->Leave += gcnew System::EventHandler(this, &frmMantAlarma::textBox2_Leave);
			   ...
			   this->comboBox2->Items->AddRange(gcnew cli::array<System::Object^>(3) { L"Snooze", L"Critica", L"" });
			   this->comboBox2->Location = Point(20, 192);
			   this->comboBox2->Name = L"comboBox2";
			   this->comboBox2->Size = System::Drawing::Size(340, 25);
			   ...
			   this->button4->Click += gcnew System::EventHandler(this, &frmMantAlarma::button4_Click);
			   ...
			   this->dataGridView1->CellContentClick += gcnew DataGridViewCellEventHandler(this, &frmMantAlarma::dataGridView1_CellContentClick);
			   ...
			   this->Column4->HeaderText = L"Duración";
			   ...
			   this->button2->Click += gcnew System::EventHandler(this, &frmMantAlarma::button2_Click);
			   ...
			   this->button1->Click += gcnew System::EventHandler(this, &frmMantAlarma::button1_Click);
			   ...
			   this->button5->Click += gcnew System::EventHandler(this, &frmMantAlarma::button5_Click);
			   ...
			   this->Name = L"frmMantAlarma";
			   this->Text = L"";
			   this->Load += gcnew System::EventHandler(this, &frmMantAlarma::frmMantAlarma_Load);
...
#pragma endregion
		   ...
		   // ── LÓGICA (sin cambios) ──────────────────────────────────────────────
	private: void mostrarGrilla(List<Alarma^>^ listaAlarmas) {
		this->dataGridView1->Rows->Clear();

		for (int i = 0; i < listaAlarmas->Count; i++) {
			Alarma^ alarma = listaAlarmas[i];

			AlarmaSnooze^ sn = dynamic_cast<AlarmaSnooze^>(alarma);
			AlarmaCritica^ cr = dynamic_cast<AlarmaCritica^>(alarma);

			array<String^>^ fila = gcnew array<String^>(4);
			fila[0] = alarma->getId().ToString();
			fila[1] = alarma->getEstado() ? "Activa" : "Inactiva";
			fila[2] = alarma->getTipo();  // "Snooze" o "Critica"

			// Duración: solo Snooze tiene tiempoDeEspera, Critica muestra guión
			fila[3] = (sn != nullptr)
				? sn->getTiempoDeEspera().ToString() + " min"
				: "-";
	...
	private: System::Void frmMantAlarma_Load(System::Object^ sender, System::EventArgs^ e) {
		mostrarGrilla(this->alarmaController->ObtenerTodasAlarmasTXT());
	}

	private: System::Void button4_Click(System::Object^ sender, System::EventArgs^ e) {
		List<Alarma^>^ todas = this->alarmaController->ObtenerTodasAlarmasTXT();
		List<Alarma^>^ resultado = gcnew List<Alarma^>();

		String^ filtroId = this->comboBox1->Text->Trim();
		String^ filtroEstado = this->textBox2->Text->Trim()->ToUpper();
		String^ filtroTipo = this->comboBox2->Text->Trim()->ToUpper();

		for each (Alarma ^ alarma in todas) {
			bool coincide = true;

			if (filtroId != "" && alarma->getId().ToString() != filtroId)
				coincide = false;

			if (filtroEstado != "") {
				String^ estadoAlarma = alarma->getEstado() ? "ACTIVA" : "INACTIVA";
				if (!estadoAlarma->Contains(filtroEstado))
					coincide = false;
			}

			if (filtroTipo != "" && !alarma->getTipo()->ToUpper()->Contains(filtroTipo))
				coincide = false;
		...
		mostrarGrilla(resultado);
	}
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		if (this->dataGridView1->SelectedRows->Count == 0) {
			MessageBox::Show("Seleccione una alarma primero.", "Aviso",
		...
		int id = Convert::ToInt32(
			this->dataGridView1->Rows[filaSeleccionada]->Cells[0]->Value->ToString()
		);

		Alarma^ alarma = this->alarmaController->BuscarAlarmaPorIdTXT(id);

		frmEliminarAlarma^ ventana = gcnew frmEliminarAlarma(this->alarmaController, alarma);
		ventana->ShowDialog();
		mostrarGrilla(this->alarmaController->ObtenerTodasAlarmasTXT());
	}
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		frmAgregarAlarma^ ventana = gcnew frmAgregarAlarma(this->alarmaController);
		ventana->ShowDialog();
		mostrarGrilla(this->alarmaController->ObtenerTodasAlarmasTXT());
	}
	private: System::Void button5_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		frmModificarAlarma^ ventana = gcnew frmModificarAlarma(this->alarmaController, alarma);
		ventana->ShowDialog();
		mostrarGrilla(this->alarmaController->ObtenerTodasAlarmasTXT());
	}
	...
	private: System::Void textBox2_Enter(System::Object^ sender, System::EventArgs^ e) {
		if (this->textBox2->Text == L"Ej. Activa") {
